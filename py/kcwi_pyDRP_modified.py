...
def kcwi_vachelio(hdu,hdr_ref='',mask=False,uncert=False,method='heliocentric'):
    ...
    if uncert:
        cube_old = cube_old**2 #python DRP generate sigma cube rather than variance cube
...
#   overwrite=False
    ...
    edge = np.array([trim_tab["col4"],trim_tab["col5"]]) #[x_edge, y_edge], edge range of the flags with wrong values
    ...
    for i in range(len(fn)):
        ...
        if (not os.path.isfile(trimfn[i])) or overwrite==True:
            ...
            #modify the edge of mask cube
            hdu_m.data[:, :, :edge[0,i]] = 128
            hdu_m.data[:, :, -edge[0,i]:] = 128
            hdu_m.data[:, :edge[1,i], :] = 128
            hdu_m.data[:, -edge[1,i]:, :] = 128

            ##following the suggestion by Don Neil in github #98, the pixels with mask =2 & 4 should be kept
            idx = np.where( (hdu_m.data ==2 ) | (hdu_m.data == 4) | (hdu_m.data == 6))
            hdu_m.data[idx] = 0
...
def kcwi_align(fnlist,wavebin=[-1.,-1.],box=[-1,-1,-1,-1],pixscale_x=-1.,pixscale_y=-1.,orientation=-1000.,dimension=[-1.,-1.],preshiftfn='',trim=[-1,-1],cubed=False,noalign=False,display=True,search_size=-1000,conv_filter=-1000,upfactor=-1000.,background_subtraction=False,background_level=-1000.,method='interp',
    ...
    edge = np.array([trimtab['col4'],trimtab['col5']]) #the edge of flags are mistakenly labelled as 0, [x_edge, y_edge].
    ...
    for i in range(len(fn)):
        ...
        #read the mask to identify the edge
        mask = hdulist['flags'].data.T.copy()

        #Need to modify the value in the edge of FLAGS to identify the bad pixels
        mask[:edge[0,i], :] = 128
        mask[-edge[0,i]:, :] = 128
        mask[:,:edge[1,i]] = 128
        mask[:,-edge[1,i]:] = 128
        ...
        for ii in range(sz[0]):
            for jj in range(sz[1]):
                q=(img[ii,jj,qwave]!=0) & np.isfinite(img[ii,jj,qwave]) &  (mask[ii,jj,qwave] <= 1e-6)
                if np.sum(q)>0:
