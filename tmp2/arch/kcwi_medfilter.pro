...
pro kcwi_medfilter,dir=dir,parfn=parfn,zbin=zbin,ybin=ybin,ytrim=ytrim,suffix=suffix,$
	overwrite=overwrite
...
for i=0,n_elements(imgnum)-1 do begin
	if fnonly[0] ne -1 then begin
		q=where(fnonly eq imgnum[i])
	...
	; get file names
	cubefn=kcwi_get_imname(kpars[i],imgnum[i],'_'+suffix,/reduced)
	if file_test(cubefn) eq 0 then continue
	mcubefn=repstr(cubefn,'_icube','_mcube')
	mimgfn1=file_dirname(cubefn)+'/'+file_basename(cubefn,'.fits')+'.mask.fits'
	...
	cube0=cube
	mcube=mrdfits(mcubefn,0,/silent)
	...
	q=where(medpar.fnnum eq imgnum[i])
...
;			goto,skipflag2
	...
	writefits,medfn,medcube,hdr
	writefits,cubefn,cube1,hdr
