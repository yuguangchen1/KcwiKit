...
if os.path.isfile('kb210415_00047_icube.fits'):

    # file names
    cubefn = 'kb210415_00047_icube.fits'
    ...
    for kk in range(shape[0]):
        for ii in range(shape[2]):
            ...
            if len(qgood) > 0:
                ...
                # lower
                smallline = line[:qmin + 4]
                smallflag = flagline[:qmin + 4]
                smallline[smallflag != 1] = np.nan
                smallflag[smallflag != 1] = -200
                # upper
                smallline = line[qmax - 4 + 1:]
                smallflag = flagline[qmax - 4 + 1:]
                smallline[smallflag != 1] = np.nan
                smallflag[smallflag != 1] = -200
    ...
    medcube = fits.open('medcube.fits')[0].data
    ...
    hdl[0].data = cube1
    hdl.writeto('kb210415_00047_icube_test.fits', overwrite=True)

    # Median cube
    hdu_med = fits.PrimaryHDU(medcube, header=hdr)
    hdu_med.writeto('kb210415_00047_icube.fits'.replace('.fits', '.med.fits'), overwrite=True)
